...
app.use(cors());
app.use(prettyJSON());
var apiDir = K(import.meta.dir, "api");
readdirSync(apiDir).forEach((version) => {
  const versionPath = K(apiDir, version);
  readdirSync(versionPath).forEach(async (resource) => {
    const resourcePath = K(versionPath, resource);
    const resourceName = resource.replace(".ts", "");
    const resourceRouter = (await import(resourcePath)).default;
    app.use(`/api/${version}/${resourceName}`, resourceRouter);
  });
});
